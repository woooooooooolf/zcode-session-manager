...
const I18N = {
  zh: {
    ...
    "about.author": "作者",
    "about.authorValue": "woooooooooolf（linyar.windforest@gmail.com）",
    "about.repo": "GitHub",
    ...
    "help.usage1": "会话列表：按标题 / ID / 项目搜索，可按归档、置顶筛选；每行的“查看”只读浏览消息流，“删除”进入两步确认（列 出级联范围、行数与备份位置）；勾选多行后可批量删除。",
    "help.usage2": "受限模式：ZCode 运行时仅可删除“已归档且闲置超过阈值”的会话（阈值在设置中调整，默认 1 小时，范围 1 分钟 ~ 365 天）；退出 ZCode 后无此限制。",
    ...
    "help.badgeA": "A = 已归档：在 ZCode 中被归档的会话（标记来自 v2\\tasks-index.sqlite）。",
    "help.badgeP": "P = 已置顶：在 ZCode 中被置顶的会话。",
    "help.badgeC": "c = 子会话：由其他会话派生（fork）而来；删除其父会话时会一并级联删除。",
    "help.badgeG": "G = 索引残影：索引中仍有记录但会话正文已不存在，可安全清理。",
  ...
  en: {
    ...
    "help.usage1": "Session list: search by title / ID / project, filter by archived or pinned; \"View\" opens a read-only message stream, \"Delete\" starts the two-step confirmation (cascade scope, row counts and backup location); select multiple rows for bulk deletion.",
    "help.usage2": "Limited mode: while ZCode is running, only sessions that are archived and idle beyond the threshold can be deleted (adjustable in Settings, default 1 hour, range 1 minute – 365 days); close ZCode to remove the restriction.",
    ...
    "help.badgeA": "A = Archived: the session was archived in ZCode (flag from v2\\tasks-index.sqlite).",
    "help.badgeP": "P = Pinned: the session is pinned in ZCode.",
    "help.badgeC": "c = Child session: forked from another session; deleting its parent also deletes it.",
    "help.badgeG": "G = Index ghost: an index row whose content is already gone; safe to clean up.",
